import EventPosterExample1 from '/events/poster/event-poster-example-1.png';
import { NavLink } from '@solidjs/router';
// import Frame13 from '../../../assets/img/frame13.svg';
// import Frame14 from '../../../assets/img/frame14.svg';
// import Frame15 from '../../../assets/img/frame15.svg';
import { createEffect, createSignal } from 'solid-js';
import { getAllEvents, getEventByJenis } from '@/functions/Event';
import {
  addBookmark,
  checkExist,
  deleteByRef,
...
export default function EventsPage() {
  const [webinar, setWebinar] = createSignal<any>([]);
  const [lomba, setLomba] = createSignal<any>([]);
  const [volunteer, setVolunteer] = createSignal<any>([]);
  const [modalMsg, setModalMsg] = createSignal<any>({});
  const [request, setRequest] = createSignal<any>(false);

  createEffect(async () => {
    const dataWebinar = await getEventByJenis('Webinar');
    const dataLomba = await getEventByJenis('Lomba');
    const dataVolunteer = await getEventByJenis('Volunteer');

    setWebinar(dataWebinar.slice(0, 3));
    setLomba(dataLomba.slice(0, 3));
    setVolunteer(dataVolunteer.slice(0, 3));
    checkBookmark();
    checkPendaftaran();
  }, []);
  ...
  const checkPendaftaran = async () => {
    ...
    Object.values(res).map((val: any) => {
      if (val.id_event) {
        document
          .getElementById(`btn-${val.id_event}`)
          ?.classList.add('bg-slate-400');
        document
          .getElementById(`btn-${val.id_event}`)
      ...
      <div class='grid min-h-screen gap-8'>
        <section>
          <div class='carousel w-full gap-6'>
            <div id='item1' class='flex w-full justify-center'>
              <Carousel />
            </div>
          </div>
        </section>
        <section>
          <div class='mb-4 flex h-16 items-center justify-between'>
            <h2 class='font-dm-sans text-2xl font-bold text-secondary'>
              Webinar
            </h2>
            <NavLink
              href={'/events/category/webinar'}
              class='font-dm-sans font-medium text-vivid-blue'
              end
            >
              Lihat Semua
            </NavLink>
              ...
              return (
                <div class='flex flex-col items-center justify-center gap-y-4 rounded-3xl bg-white p-5 dark:bg-gray-800'>
                    ...
                    <div class='absolute inset-0 overflow-hidden transition-opacity group-hover:opacity-80'>
                      ...
                      <button class='absolute inset-0 flex h-full w-full items-center justify-center bg-black bg-opacity-50 opacity-0 transition-opacity group-hover:opacity-100'>
                        <a
                          ...
                          href={`/events/category/webinar/${val.id_event}`}
                        >
                          <p>Lihat Kegiatan</p>
                        </a>
                      </button>
                  ...
                  <button
                    ...
                    onClick={() => {
                      handlePopUp(val.id_event, val.judul_event);
                    }}
                  >
                    Ikut Kegiatan
                  </button>
